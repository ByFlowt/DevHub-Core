...
import {
    Blocks, ChevronLeft, ChevronRight,
    Container,
    Folder,
    FolderGit2,
    HatGlasses,
...
const Sidebar = () => {
    ...
    const items = [
        ...
        {id: 3, name: "Pipelines", icon: <NotepadTextDashed />, path: "/pipelines"},
...
/*branch test*/
    return (
        <div>
            <button aria-label="Toggle Sidebar" className="md:hidden fixed top-4 left-4 z-50 p-2 rounded-lg shadow-lg" onClick={() => setIsOpen(!isOpen)}>
                {isOpen ? <Menu  size={18}/> : <Menu size={18}/>}
            </button>

            <aside className={`fixed top-0 left-0 h-full bg-black shadow-xl transition-all duration-300 ease-in-out z-40 ${isOpen ? "w-64 translate-x-0" : "w-64 -translate-x-full md:w-20 md:translate-x-0"}`}>
                <div className="flex flex-col h-full">
                    ...
                    <button aria-label="Toggle Sidebar" className="hidden md:flex absolute -right-3 top-20 bg-white rounded-full p-1.5 border border-gray-200  cursor-pointer shadow-lg" onClick={() => setIsOpen(!isOpen)}>
